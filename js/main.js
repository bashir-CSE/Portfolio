...
const PortfolioApp = {
    ...
    handleContactFormSubmit(e) {
        ...
        emailjs.send(this.config.emailjs.serviceID, this.config.emailjs.templateID, templateParams)
            .then(() => {
                this.elements.formMessage.innerHTML = '<div class="p-4 bg-green-100 border border-green-400 text-green-700 rounded"><i class="fas fa-check-circle mr-2"></i>Message sent successfully!</div>';
                this.elements.contactForm.reset();
            })
            .catch((error) => {
                console.error('EmailJS error:', error);
                this.elements.formMessage.innerHTML = `<div class="p-4 bg-red-100 border border-red-400 text-red-700 rounded"><i class="fas fa-exclamation-circle mr-2"></i>${error.text || 'Unable to send message.'}</div>`;
            })
            .finally(() => {
                ...
                setTimeout(() => this.elements.formMessage.classList.add('hidden'), 5000);
            });
    },
    ...
    initTyping() {
        new Typed('#typed-text', this.config.typing);
    },
    ...
    initIntersectionObserver() {
        ...
        // Observer for skill bar animations
        ...
        this.elements.skillItems.forEach(item => skillBarObserver.observe(item));
    },
